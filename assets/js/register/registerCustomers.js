class Customer{
    ...
    toSave() {
        let custumer =  this.readData();

        if(this.validateField(custumer)) {
            if(this.editId == null) {
                this.toAdd(custumer);
            } else {
                this.update(this.editId, custumer);
            }
        }
    ...
    toAdd(custumer) {
        this.arrayCustomers.push(custumer);
        this.id++;
        
    }

    delete(id, customerName) {
        if(confirm("Deseja realmente deletar os dados do cliente " + customerName)) {
            let tbody = document.getElementById('tbody');

            for(let i = 0; i < this.arrayCustomers.length; i++ ) {
                if(this.arrayCustomers[i].id == id) {
                    this.arrayCustomers.splice(i, 1);
                    tbody.deleteRow(i);
                }
            }
        }
